...
# Speech: Bonus at the end, compare this typical route on different runtimes (quarkus, spring-boot)
...
mvnu io.quarkus:quarkus-maven-plugin:1.7.2.Final:create
...
# Speech: Camel Quarkus Bootstrap, one way is to extend RouteBuilder
...
# JVM vs Native ~= It's different optimizations adapted to different scenarios.
  ...
  Native ~= long build/debug, instant startup, less memory, some short running jvms. lower throughput per-jvm but you can scale.
...
# Then run compare.sh
cd ..
# Speech: Bonus Time ! Compare this demo route on different runtimes
...
TODO: Maybe start with a non arc RouteBuilder, then say oh I need a bean, or whatnot, so add @ApplicationScoped on the route. And actually, is there any scenarios where we would take the boot time penalty of using @ApplicationScoped route ?
...
TODO ABEL: Take care with bluejeans, shift of 2/3s between audio/video, a small pause between command
           Show that test were executed first, then transition to this is JVM MODE.
           Don't talk about Bonus at the end (this is just he last step)
           Enunciate the plan clearly at the beginning (and then good transition between phases)
           Number of extensions ~50% of camel components, complete list on quarkus site, jvm only or native
           Camel quarkus offer XML DSL
           Use of env variable for setting the quarkus property
